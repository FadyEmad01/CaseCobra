...
const Page = () => {
  ...
  const [updateProgress, setUploadProgress] = useState<number>(0);
  const [fileAccepted, setFileAccepted] = useState<boolean>(false); // New state to track if a file is accepted
  ...
  const onDropAccepted = (acceptedFile: File[]) => {
    ...
    setIsDragOver(false);
    setFileAccepted(true); // Set the fileAccepted state to true when a file is accepted
  };
